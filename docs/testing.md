...
| Sidebar publication — `nqtc` | Width for expanded/icons/hidden combinations; `SidebarWidthCapture.accepted` records a user-dragged width only when expanded, finite, and in range (rejects collapsed/programmatic and sub-minimum drags, clamps above the maximum) | Actual Combine emissions drive current divider width; hidden-to-icons and icons-to-hidden update immediately; expand via menu/shortcut; preserve surfaces. Restored-collapsed sidebar applies the collapsed width on the first `$isCollapsed` emission (no expanded-width flash); restored-expanded resumes at the persisted width |
...
3. Collapse to icons, change to hidden, then expand. Drag the divider to resize the
   ...
   sidebar resumes its collapsed/expanded state and its last dragged width.
